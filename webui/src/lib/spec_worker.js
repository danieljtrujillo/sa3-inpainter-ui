...
self.onmessage = (e) => {
  ...
  if (m.op === "render") {
    const { token, target, audio, sampleRate, width, height,
            ...
            tonalRatio = 2.2,        // looser tonality test → high-freq broadband content keeps body
            cifLimit = 0.5,
            smoothAttenDb = 7,        // less dimming of smooth backdrop → noise/cymbals/hi-hats survive
          } = m;
    try {
