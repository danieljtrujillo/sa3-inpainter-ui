...
function paintFromCache() {
  ...
  // No-op if the tile we'd paint is the one already on screen.
  const cur = untrack(() => renderedRange);
  if (Math.abs(t.range.start - cur.start) < 1e-9 &&
      Math.abs(t.range.end   - cur.end)   < 1e-9) return false;
  // Promote in LRU
  ...
  // drawImage paints fully-opaque pixels over whatever was there — no clearRect
  // needed, which keeps the blit truly atomic (no transient blank).
...
$effect(() => {
  // Live zoom/pan changes: CSS transform is smooth between frames. When the
  // current renderedRange no longer suits the visible window, immediately swap
  // in the best CACHED tile (instant — no worker, no flash) so the user sees a
  // tile that fits rather than a stretched/stale one. Then debounce a fresh
  // worker render for the precise version.
  // untrack renderedRange so the writes inside paintFromCache don't re-fire us.
