...
// fetch memory token info when model is loaded
$effect(() => {
  ...
  <section class="cta-panel">
    {#if ctaVisible || session.generating}
      <button class="btn btn-primary btn-lg" onclick={clickGenerate}
              title={session.generating ? "Cancel the running generation" : `Run ${ctaLabel.toLowerCase()} with the current settings`}>
    ...
    {#snippet children()}
      ...
      <div class="form-row">
        ...
        <div class="seed-row">
          ...
          <button class="icon-btn" onclick={repeatSeed}
                  title="Re-use the last seed (reproduce the previous generation)"
                  disabled={session.lastSeed == null}>
            <i class="bi bi-arrow-repeat"></i>
          </button>
          <button class="icon-btn" onclick={rerollSeed}
                  title="Pick a new random seed (does not generate yet)">
            <i class="bi bi-dice-5"></i>
          </button>
        </div>
      </div>
